...
const activeTab = ref('open'); // 'open' | 'private'
const location = ref('');
        ...
        <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
            ...
            <div class="max-w-5xl mx-auto -mt-16 sm:-mt-22 md:-mt-26 lg:-mt-28 relative z-20 px-1 sm:px-4">
                <div class="bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-100 p-3.5 sm:p-5 md:p-6 backdrop-blur-xs">
                    <!-- Top Tab Switcher & All-Inclusive Tagline -->
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 border-b border-gray-100 pb-3.5">
                        <div class="flex items-center gap-2">
                            <!-- Tab 1: Paket Open Trip -->
                            <button
                                type="button"
                                @click="selectTab('open')"
                                :class="[
                                    'flex items-center justify-center gap-1.5 sm:gap-2 px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-bold transition cursor-pointer',
                                    activeTab === 'open'
                                        ? 'bg-blue-50 text-[#0052cc] ring-1 ring-blue-200 shadow-2xs'
                                        : 'text-slate-600 hover:text-slate-900 hover:bg-gray-50'
                                ]"
                            >
                                <div :class="['w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center shrink-0', activeTab === 'open' ? 'bg-[#0052cc] text-white' : 'bg-blue-100 text-[#0052cc]']">
                                    <Users class="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                                </div>
                                <span class="whitespace-nowrap">Paket Open Trip</span>
                            </button>

                            <!-- Tab 2: Paket Private Trip -->
                            <button
                                type="button"
                            ...
                            >
                                <div :class="['w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center shrink-0', activeTab === 'private' ? 'bg-emerald-600 text-white' : 'bg-emerald-100 text-emerald-700']">
                                    <Sparkles class="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                                </div>
                                <span class="whitespace-nowrap">Paket Private Trip</span>
                            </button>
                        </div>

                        <!-- All Inclusive Callout Badge -->
                        <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 border border-amber-200 text-amber-800 text-[11px] sm:text-xs font-semibold">
                            <CheckCircle2 class="w-3.5 h-3.5 text-amber-600 shrink-0" />
                            <span>Sudah Termasuk: <strong>Penginapan</strong> + <strong>Kendaraan</strong> + <strong>Bebas Pilih Makanan</strong></span>
                        </div>
                    </div>
