    ...
    <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <article v-for="trip in trips" :key="trip.id" class="panel-surface">
            <Link :href="route('trips.show', [trip.type, trip.slug])" class="group block h-full">
                ...
                <div class="p-5"><p class="text-[10px] uppercase tracking-wider text-slate-400">{{ trip.vendor.name }}</p><h2 class="mt-2 text-base font-bold text-[#17375f] group-hover:text-blue-600">{{ trip.title }}</h2><p class="mt-2 flex items-center gap-1 text-xs text-slate-500"><MapPin class="size-3" />{{ trip.destination }}</p><p class="mt-2 flex items-center gap-1 text-xs text-slate-500"><CalendarDays class="size-3" />{{ trip.departure_date.slice(0, 10) }} · Sisa {{ Math.max(0, trip.capacity - trip.reserved_seats) }} tempat</p><div class="mt-5 flex items-center justify-between"><p class="font-bold text-blue-600">{{ money(trip.price) }} <span class="text-xs font-normal text-slate-400">/ orang</span></p><ArrowUpRight class="size-5 text-blue-600" /></div></div>
            </Link>
        </article>
    </div>
