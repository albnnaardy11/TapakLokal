...
<template>
    <section aria-labelledby="profile-heading" class="text-[#26364d]">
        <h2 id="profile-heading" class="text-sm font-extrabold text-[#183660]">Pengaturan Akun</h2><p class="mt-1 text-[11px] leading-5 text-slate-500">Kelola informasi pribadi, kontak, dan keamanan akun perjalananmu.</p>
        <div class="mt-4 inline-flex gap-1 rounded-full border border-[#e1eaf5] bg-white p-1.5 shadow-[0_3px_12px_rgba(30,90,160,0.05)]"><button v-for="item in ['Profil', 'Keamanan']" :key="item" class="inline-flex min-h-8 items-center gap-2 rounded-full px-4 py-1.5 text-[10px] font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-[#3E7BEF]" :class="tab === item ? 'bg-[#3E7BEF] text-white shadow-sm' : 'text-slate-500 hover:bg-sky-50'" :aria-pressed="tab === item" @click="tab = item"><component :is="item === 'Profil' ? UserRound : ShieldCheck" class="size-3.5" />{{ item }}</button></div>
            ...
            <form class="mt-4 overflow-hidden rounded-2xl border border-[#e1eaf5] bg-white shadow-[0_4px_20px_rgba(23,75,120,0.04)]" @submit.prevent="save">
                ...
                <div class="p-4 sm:p-5">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <label class="flex min-w-0 flex-col gap-2 text-xs font-semibold">Tanggal lahir<input v-model="form.birth" type="date" :max="new Date().toLocaleDateString('en-CA')" autocomplete="bday" class="h-11 min-w-0 rounded-xl border border-[#e1eaf5] bg-white px-3 text-xs font-normal outline-[#078cff]" /></label>
                        <label class="flex flex-col gap-2 text-xs font-semibold sm:col-span-2">Kota tempat tinggal<input v-model="form.city" maxlength="100" autocomplete="address-level2" placeholder="Contoh: Jakarta Timur" class="h-11 rounded-xl border border-[#e1eaf5] px-3 text-xs font-normal outline-[#078cff]" /></label>
                    </div>
                </div>
                <div class="flex justify-end gap-2 border-t border-slate-100 bg-[#fbfcfe] px-4 py-3"><button type="button" class="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-500 hover:bg-slate-50" @click="Object.assign(form, saved)">Batalkan</button><button class="inline-flex items-center gap-2 rounded-lg bg-[#078cff] px-4 py-2.5 text-xs font-semibold text-white hover:bg-[#0878db]"><Check class="size-4" />Simpan perubahan</button></div>
            </form>
            <div v-for="section in [{ type: 'email', title: 'Alamat email', icon: Mail, items: emails }, { type: 'phone', title: 'Nomor ponsel', icon: Phone, items: phones }]" :key="section.type" class="mt-4 rounded-2xl border border-[#e1eaf5] bg-white shadow-[0_4px_20px_rgba(23,75,120,0.04)] p-4 sm:p-5">
                <div class="flex items-start gap-3"><span class="grid size-10 shrink-0 place-items-center rounded-xl bg-[#edf5ff] text-[#3E7BEF]"><component :is="section.icon" class="size-5" /></span><div class="flex-1"><h3 class="text-xs font-bold text-[#183660]">{{ section.title }}</h3><p class="mt-1 text-[11px] text-slate-400">Kelola hingga 3 kontak untuk akunmu.</p></div><button :disabled="section.items.length >= 3" class="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-xs font-semibold text-[#078cff] hover:bg-sky-50 disabled:opacity-40" @click="addContact(section.type)"><Plus class="size-4" />Tambah</button></div>
                ...
                <div v-else class="mt-4 rounded-xl border border-dashed border-[#e1eaf5] px-4 py-5 text-center"><p class="text-xs text-slate-500">Belum ada {{ section.title.toLowerCase() }} ditambahkan.</p><p class="mt-1 text-[10px] text-slate-400">Tambahkan kontak agar informasi perjalanan mudah dijangkau.</p></div>
            </div>
        </template>
        <AccountSecurity v-else /><AccountExploreBanner />
