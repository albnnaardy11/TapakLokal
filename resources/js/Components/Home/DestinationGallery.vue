...
<template>
    <section
        class="mx-auto mt-20 grid max-w-[1180px] gap-2.5 text-[#172c70] sm:mt-24 lg:grid-cols-[704fr_462fr]"
        aria-labelledby="destination-gallery-heading"
        :aria-busy="isLoading"
    >
        <!-- Skeleton State -->
        <template v-if="isLoading">
            <div class="min-w-0 rounded-2xl bg-white/95 p-5 sm:p-6 lg:min-h-[560px] space-y-4">
                <div class="flex items-center gap-3">
                    <SkeletonBox width="44px" height="44px" rounded="rounded-xl" />
                    <div class="space-y-1.5 flex-1">
                        <SkeletonBox width="200px" height="18px" rounded="rounded-md" />
                        <SkeletonBox width="70%" height="12px" rounded="rounded-md" />
                    </div>
                </div>
                <SkeletonBox height="260px" rounded="rounded-xl" />
                <div class="flex gap-2.5 pt-2">
                    <SkeletonBox v-for="n in 4" :key="n" height="96px" rounded="rounded-xl" class="flex-1" />
                </div>
            </div>
            <div class="rounded-2xl bg-white/95 p-5 sm:p-6 space-y-3 lg:h-[560px]">
                <SkeletonBox height="42px" rounded="rounded-xl" />
                <div class="space-y-2.5">
                    <SkeletonBox v-for="n in 4" :key="n" height="72px" rounded="rounded-xl" />
                </div>
            </div>
        </template>
