...
<template>
    <section class="mx-auto mt-20 max-w-[1180px] sm:mt-24" aria-labelledby="travel-blog-heading">
        ...
        <div ref="carousel" class="mt-7 flex snap-x snap-mandatory gap-5 overflow-x-auto pb-5 pt-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" @scroll="syncPage">
            <article v-for="article in filteredArticles" :key="article.id" class="group flex w-[85%] shrink-0 snap-start flex-col overflow-hidden rounded-2xl border border-[#e1eaf3] bg-white shadow-[0_5px_18px_rgba(23,75,120,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_14px_30px_rgba(23,100,180,0.12)] motion-reduce:transform-none motion-reduce:transition-none sm:w-[calc((100%-20px)/2)] lg:w-[calc((100%-60px)/4)]">
                <button type="button" class="flex h-full flex-col text-left outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#078cff]" :aria-label="`Baca ${article.title}`" @click="readArticle(article)">
                    ...
                    <div class="flex flex-1 flex-col p-5">
                        ...
                        <span class="mt-auto flex justify-end pt-6"><span class="inline-flex h-9 items-center gap-2 rounded-lg bg-[#edf6ff] px-3 text-xs font-bold text-[#078cff] transition-colors duration-200 hover:bg-[#078cff] hover:text-white">Baca cerita <ArrowRight class="size-3.5" aria-hidden="true" /></span></span>
                    </div>
                </button>
            </article>
        </div>
