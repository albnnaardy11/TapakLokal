...
<template>
    <section class="absolute -bottom-14 left-1/2 z-20 w-[calc(100%-2rem)] max-w-[1120px] -translate-x-1/2 rounded-2xl border border-[#dce7f4] bg-white p-3 shadow-[0_18px_42px_rgba(17,54,92,0.20)] sm:w-[calc(100%-4rem)] sm:p-4">
        ...
        <form class="grid gap-2 rounded-xl bg-[#f5f8fc] p-2 sm:grid-cols-[minmax(0,1.7fr)_minmax(0,0.95fr)_minmax(0,1.05fr)_auto]" @submit.prevent="searchTrips">
            ...
            <label class="flex min-w-0 items-center gap-2 rounded-lg border border-transparent bg-white px-3 py-3 text-left text-xs text-slate-400 shadow-[0_1px_4px_rgba(29,73,120,0.08)] transition focus-within:border-[#91c5fc] focus-within:ring-2 focus-within:ring-[#dceeff]"><ShoppingBag class="size-4 shrink-0 text-[#5f86b4]" /><select v-model="category" class="min-w-0 flex-1 appearance-none bg-transparent text-xs text-slate-600 outline-none" aria-label="Kategori"><option value="">Semua Kategori</option><option value="wisata-alam">Wisata Alam</option><option value="budaya">Budaya & Sejarah</option><option value="kuliner">Kuliner Lokal</option><option value="petualangan">Petualangan</option></select><ChevronDown class="size-4 shrink-0" /></label>
