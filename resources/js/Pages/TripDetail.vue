...
const detail = computed(() => {
    const fallback = isPrivateTrip.value ? privateTrip : openTrip;
    if (! props.tripData) {
        return fallback;
    }

    const dynamicImages = [props.tripData.image_url, ...(props.tripData.experience?.detail?.images || [])].filter(Boolean);
    const durationDays = Math.max(1, Math.round((new Date(props.tripData.end_date || props.tripData.departure_date) - new Date(props.tripData.departure_date)) / 86400000) + 1);

    return {
        ...fallback,
        title: props.tripData.title,
        startPoint: props.tripData.meeting_point,
        duration: `${durationDays}H${Math.max(0, durationDays - 1)}M`,
            ...
            <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_330px] lg:items-start">
                <section class="min-w-0">
                    <div class="mb-5 sm:mb-6">
                        ...
                        <p class="mt-3 max-w-2xl text-xs leading-5 text-[#60789c] sm:text-sm">{{ detail.subtitle }}</p>
                    </div>
                    <section class="relative isolate overflow-hidden rounded-[28px] bg-[#0d73d4] p-3 shadow-[0_18px_42px_rgba(18,95,178,0.18)] sm:p-4" aria-label="Galeri perjalanan">
                ...
                <aside class="self-start lg:fixed lg:top-[118px] lg:right-[max(2rem,calc((100vw-1220px)/2))] lg:z-20 lg:w-[330px]"><section class="rounded-2xl border border-[#dce7f4] bg-white p-5 shadow-[0_8px_28px_rgba(23,75,120,0.08)]"><p class="text-xs text-slate-400 line-through">{{ detail.originalPrice }}</p><div class="mt-1 flex items-end gap-2"><p class="text-2xl font-extrabold tracking-tight text-[#173b70]">{{ detail.price }}</p><span class="pb-1 text-[10px] text-slate-500">/ orang</span></div><p class="mt-1 text-[11px] text-slate-500">Harga dapat berubah sesuai jadwal dan jumlah peserta.</p><div class="mt-4 rounded-xl bg-[#f5f9ff] p-3"><p class="text-[10px] font-semibold text-[#627a99]">Keberangkatan</p><p class="mt-1 flex items-center gap-2 text-xs font-bold text-[#31577f]"><CalendarDays class="size-4 text-[#1677e8]" />{{ detail.date }}</p></div><button type="button" class="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#1677e8] px-4 text-xs font-bold text-white shadow-[0_5px_12px_rgba(22,119,232,0.25)] transition hover:bg-[#0d68d1]" @click="checkAvailability"><CalendarDays class="size-4" />Cek ketersediaan</button><p class="mt-3 flex items-center gap-1.5 text-[10px] text-[#1677e8]"><Coins class="size-3.5 text-amber-500" />Dapatkan hingga 400 poin TapakLokal</p></section>
