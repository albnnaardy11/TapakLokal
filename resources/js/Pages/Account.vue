...
import AccountPoints from '../Components/Account/AccountPoints.vue';
import AccountVouchers from '../Components/Account/AccountVouchers.vue';
import AccountProfile from '../Components/Account/AccountProfile.vue';
import AccountCommunity from '../Components/Account/AccountCommunity.vue';
import AccountWallet from '../Components/Account/AccountWallet.vue';
import AccountExplore from '../Components/Account/AccountExplore.vue';
    ...
    <div class="min-h-screen overflow-x-hidden bg-[#f7f9fb] text-[#303e4c]">
        ...
        <div class="mx-auto grid max-w-[1220px] gap-5 px-4 py-5 sm:px-5 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-7">
            ...
            <main class="min-w-0" :aria-busy="loading"><p v-if="navigationError" role="alert" class="mb-4 rounded-xl bg-rose-50 p-4 text-sm text-rose-600">{{ navigationError }}</p><div v-if="loading" role="status" class="space-y-4"><span class="sr-only">Memuat halaman akun…</span><div aria-hidden="true" class="space-y-4 animate-pulse motion-reduce:animate-none"><div class="h-7 w-48 rounded-lg bg-slate-200"></div><div class="h-32 rounded-2xl bg-slate-200/70"></div><div class="grid grid-cols-2 gap-3"><div v-for="n in 4" :key="n" class="h-24 rounded-xl bg-slate-200/60"></div></div><div class="h-56 rounded-2xl bg-slate-200/60"></div></div></div><div v-show="!loading">
                <section class="relative isolate mb-5 flex min-h-[150px] flex-col justify-center overflow-hidden rounded-xl bg-[#123b53] px-5 py-5 text-white sm:min-h-[164px] sm:px-6" aria-labelledby="account-dashboard-banner-heading"><img :src="mountain" alt="Pegunungan Indonesia" class="absolute inset-0 -z-20 size-full object-cover" /><div class="absolute inset-0 -z-10 bg-gradient-to-r from-[#123b53]/90 via-[#123b53]/35 to-transparent"></div><h2 id="account-dashboard-banner-heading" class="text-lg font-extrabold">Perjalanan baru, cerita baru.</h2><p class="mt-1 text-[9px] text-white/80">Temukan pengalaman tak terlupakan di setiap sudut Indonesia.</p><Link href="/" class="mt-3 inline-flex w-fit items-center gap-3 rounded-full bg-white px-3.5 py-2 text-[9px] font-bold text-[#175a9f]">Jelajahi Destinasi<ChevronRight class="size-3" /></Link></section>
