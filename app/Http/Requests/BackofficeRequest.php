...
class BackofficeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($this->route('module') === 'trips') {
            $rules['departure_date'] = ['required', 'date', 'after_or_equal:today'];
            $rules['end_date'] = ['required', 'date', 'after_or_equal:departure_date'];
            $rules['capacity'] = ['required', 'integer', 'between:1,10000'];
            $rules['price'] = ['required', 'integer', 'between:1000,100000000'];
        }
        ...
        if (isset($rules['slug'])) {
            $rules['slug'][] = 'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/';
        }
        if (isset($rules['ends_at'])) {
            $rules['ends_at'][] = 'after_or_equal:starts_at';
        }
        if (isset($rules['user_id'])) {
            $rules['user_id'][] = 'exists:users,id';
            $rules['user_id'][] = Rule::unique('affiliates', 'user_id')->ignore($record);
        }
        if (isset($rules['commission_bps'])) {
            $rules['commission_bps'] = ['required', 'integer', 'between:0,10000'];
        }
        if ($this->route('module') === 'promotions' && $this->input('type') === 'percent') {
