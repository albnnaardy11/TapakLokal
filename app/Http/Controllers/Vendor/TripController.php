...
{
    public function create(): Response { return Inertia::render('Vendor/TripForm', ['trip' => null]); }
    ...
    public function update(TripRequest $request, Trip $trip, AuditService $audit): RedirectResponse
    {
        ...
        DB::transaction(function () use ($request, $trip, $audit) {
            ...
            if ($trip->bookings()->exists()) {
                throw \Illuminate\Validation\ValidationException::withMessages(['title' => 'Trip yang sudah memiliki pesanan tidak dapat diubah. Buat jadwal baru atau hubungi operasional.']);
            }
