...
class MediaController extends Controller
{
    public function store(Request $request, AuditService $audit): RedirectResponse
    {
        ...
        $data = $request->validate(['file' => ['required', 'file', 'mimes:jpg,jpeg,png,webp,pdf', 'max:5120'], 'alt_text' => ['nullable', 'string', 'max:255'], 'visibility' => ['required', \Illuminate\Validation\Rule::in(['public', 'private'])]]);
        abort_if($data['visibility'] === 'public' && ! $request->user()->hasPermission('content.manage'), 403);
        if ($data['visibility'] === 'public') { $request->validate(['file' => ['image', 'mimes:jpg,jpeg,png,webp']]); }
    ...
    {
        $user = $request->user();
        $canVerify = $user?->hasPermission('vendor.verify') && \App\Models\Vendor::where('document_id', $media->id)->exists();
